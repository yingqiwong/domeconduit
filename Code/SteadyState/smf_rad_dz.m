...
o.slv.fd          = 1e-6;
o.slv.fdo         = 2;
o.slv.zero.tolx   = 1e-5;           % fzero tolerance
o.slv.de.reltol   = 1e-5;           % relative tolerance
o.slv.de.abstol   = 1e-10;          % absolute tolerance
o.slv.verbose     = 1;              % print solver statements
o.slv.max_time    = 60;             % Max time [s] per solve attempt.
o.slv.use_mex     = 1;          	% use mex code?
o.slv.mexFunc     = 'smf_rad_mex';  % which mex code to use (depends on os)

% model parameters
o.Nz              = 1001;           % number of points in depth
o.v_init_exp      = -4;             % initial velocity guess
...
o.R               = 100;            % conduit radius [m]
o.conduit_length  = 3500;           % conduit length [m]

% Permeability model
o.k_lat_model     = 2;              % permeability model
o.L_diff          = 1;              % lat perm scale [factor of R]
o.k_lat           = 1e-15;          % magma perm scale (kc in paper) [m2]
o.phi_gc          = 0.3;            % percolation threshold
o.k_lat_wall.L    = 100;            % length scale for pw-phyd [m]
o.k_lat_wall.top  = inf;            % wall rock perm at surface
o.k_lat_wall.mi   = 3.2;            % Manning-Ingebritsen exponent
o.use_k_vert      = 1;              % vertical permeability on?
o.eta_g           = 1e-4;           % gas viscosity
o.plug_gas_loss   = 1;              % Plug gas loss. If used, overrides k_lat when F < 1.


o.model = 'newtonian';

% The nominal model: o.model = 'bingham';
...
% is modified to
%     vv + vf = v.
% Yield strength for bingham model
o.ys.phi0 = 0.5;
o.ys.phim = 0.75;
...
% Magma rheology options
o.eta.melt        = 2;      % viscosity model: 1=Hess&Dingwell; 2=Whittington
o.use_phi_s_ratio = 0;    	% how to calculate viscosity with solids
o.eta.use_strain  = 1;      % 1 = using strain dependent rheology

% Parameters for friction
o.fr.v_ref  = 0.05;         % reference velocity [m/s]
o.fr.f0     = 0.4;          % nominal coeff of friction
o.fr.a      = 0.1;          % friction rate-dependence
end
...
% model parameters
m.ch                = o.ch;                 % chamber parameters
m.R                 = o.R;                  % conduit radius   [m]
m.g                 = 9.81;                 % gravity          [m/s^2]
m.conduit_length    = o.conduit_length;     % conduit length   [m]
m.chi_ch.total.h2o  = o.total_h2o;          % chamber h2o      [wt%]
m.chi_ch.total.co2  = o.total_co2;          % chamber co2      [ppm]

% temperature of solution 
Ttable  = [950; 900; 850; 800; 750];    % [deg C]
m.Ti    = o.Tindex;                     % which temp is the solution at?
m.T     = Ttable(m.Ti) + 273.15;        % change to Kelvin
m.pf    = pfl_init(m.Ti);               % load solid fraction table at that temp

% densities [all kg/m^3]
rho         = 2500;         % ??
rho_litho   = 2700;         % crust 
rho_wat     = 1000;         % water
m.rho_l     = 2200;         % melt 
m.rho_hd    = m.rho_l;      % dissolved h2o
m.rho_cd    = m.rho_l;      % dissolved co2
m.rho_s     = 2600;         % solids
m.rho_c     = 741;          % ??

m.Rw        = 461.5000;     % gas constant of water
m.Rc        = 188.9000;     % gas constant of co2
m.B         = 18.02/44.01;  % molar mass ratio water/co2

% normal stress
m.sig.slope  = (rho_litho - rho_wat)*m.g;    % normal stress [Pa]
m.sig.offset = 1e6;     %Add some surface parallel stress so > 0 [Pa]
...
if (isempty(o.p_ch))
    ...
    if (m.v_ch_guess < 0), m.v_ch_guess = 1e-4; end
end
...
m.sF  = [m.p_ch; v_ch*[0.5*(m.rho_l + m.rho_s); m.rho_l; m.rho_l]]/m.conduit_length;
...
phi_s           = phi_s_of_p*m.rho_l*c12*(1 - phi_g)/(m.rho_s + phi_s_of_p*(c12*m.rho_l - m.rho_s));
